...
var FormularioContactoComponent = (function () {
    function FormularioContactoComponent(_contactosService) {
        ...
        this.rutaAvatar = '/images/ring.svg';
    }
    ...
    FormularioContactoComponent.prototype.inputTelefonoFocus = function (event) {
        var val = event.target.value;
        var re = /\(?(\d{3})\)?[- ]?(\d{3})[- ]?(\d{3})/g;
        var subst = '$1 $2 $3';
        event.target.value = val.replace(re, subst);
    };
    FormularioContactoComponent.prototype.guardarContacto = function (contactoForm) {
        var contacto = contacto_1.Contacto.jsonToContacto(contactoForm.value);
